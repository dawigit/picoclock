...
static void LCD_1IN28_InitReg(void)
{
  ...
  LCD_1IN28_SendCommand(0xFE);
	...
	LCD_1IN28_SendData_8Bit(0x08);//bgr

	LCD_1IN28_SendCommand(0x3A);
	LCD_1IN28_SendData_8Bit(0x55);//16bits/pixel
...
void LCD_1IN28_BLIT(UWORD Xstart, UWORD Ystart, UWORD Xend, UWORD Yend, UWORD *Image)
{
    // display
    UDOUBLE Addr = 0;
    ...
    UWORD j;
    uint16_t i=0;
    uint16_t xs=Xend-Xstart;
    uint16_t ys=Yend-Ystart;
    LCD_1IN28_SetWindows(Xstart, Ystart, Xend , Yend);
    DEV_Digital_Write(LCD_DC_PIN, 1);;
    for (j = 0; j < ys; j++) {
        Addr = j * xs ;
        DEV_SPI_Write_nByte((uint8_t *)&Image[Addr], xs<<1);
    }
...
void LCD_1IN28_SLEEPON(){
  LCD_1IN28_SendCommand(0x10);

};
void LCD_1IN28_SLEEPOFF(){
  LCD_1IN28_SendCommand(0x11);
};
